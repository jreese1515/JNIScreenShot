...
//remove this after removing cout
#include <iostream>
using namespace std;
...
HRESULT CDXGICapture::CaptureToFile(_In_ LPCWSTR lpcwOutputFileName, int x, int y, int width, int height)
{
	...
	*/
	...
	D2D1_RECT_F rcSource2 = D2D1::RectF(
		(FLOAT)m_rendererInfo.OutputSize.Width / 4.0f,
		(FLOAT)m_rendererInfo.OutputSize.Height / 4.0f,
		(FLOAT)1756,
		(FLOAT)1115);
	...
	m_ipD2D1RenderTarget->DrawBitmap(ipD2D1SourceBitmap, rcSource2, 1.0f, D2D1_BITMAP_INTERPOLATION_MODE_LINEAR, rcSource);
	// Reset transform
